...
%% =====================================================================
%%  Defensa de TFM — Influencia del codificador visual en Diffusion Policy
...
\subtitle{Entrenar la ResNet-18 desde cero supera al preentrenamiento de ImageNet en Push-T}
...
\begin{frame}{Diffusion Policy resolvió el generador; el codificador sigue sin decidirse}
  ...
  \end{itemize}
  \begin{keypoint}
    \textbf{Pregunta:} con \num{206} demostraciones, ¿preentrenar?
...
\begin{frame}{La respuesta corta: el preentrenamiento de ImageNet estorba aquí}
  \begin{enumerate}
    \item \textbf{Diseño.} Cinco codificadores en la misma política; lo demás, idéntico.
    \item \textbf{Resultado.} Desde cero \hl{\num{0.864}}; congelado \num{0.668};
          ajustado \num{0.648}.
    \item \textbf{Coste.} La línea base, además, \num{3.5} y \num{9.8} veces más barata
          por época.
  \end{enumerate}
...
% ---------------------------------------------------------------- 9. tabla contrastes
\begin{frame}{Las dos diferencias frente a la línea base excluyen el cero}
  \begin{center}
  \begin{tabular}{l S[table-format=1.3] c l}
    \toprule
    & {Mejor puntuación} & IC \SI{95}{\percent} de la diferencia & Wilcoxon \\
    \midrule
    \textbf{V0} desde cero & \bfseries 0.864 & {} & {} \\
    V1 congelado    & 0.668 & [\num{0.091};\,\num{0.303}] & $p=\num{0.0004}$ \\
    V2 ajuste fino  & 0.648 & [\num{0.102};\,\num{0.332}] & $p=\num{0.0001}$ \\
    \midrule
    V1 frente a V2  & \fade{0.020} & \fade{[\num{-0.093};\,\num{0.133}]} &
      \fade{$p=\num{0.82}$} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \src{Diferencias pareadas sobre los \num{50} episodios comunes. V0: época \num{350};
    V1 y V2: época \num{150}.}
  \takeaway{Caídas del \SI{23}{\percent} y del \SI{25}{\percent}. Entre congelar y ajustar,
    en cambio, no hay diferencia medible.}
\end{frame}
...
\begin{frame}{Congelar no abarata nada si obliga a subir la imagen a \num{224} píxeles}
  ...
  \begin{tabular}{l S[table-format=2.1] S[table-format=2.1] S[table-format=3.1]}
    ...
    \midrule
    \textbf{V0} \fade{(\num{96} px)}  & \bfseries 1.5 & {--} & \bfseries 17.4 \\
    V1 \fade{(\num{224} px)}          & 5.3  & 3.5 & 96.5 \\
    V2 \fade{(\num{224} px)}          & 14.7 & 9.8 & 84.9 \\
    \bottomrule
  \end{tabular}
  \end{center}
  \src{V2 acumula menos horas solo porque se detuvo en la época \num{266}.
    Total del experimento: \SI{198.8}{\hour} sobre una RTX 3070 Ti de \num{8} GB.}
  \takeaway{V1 entrena un \SI{4}{\percent} menos de parámetros y aun así cuesta tres veces
    y media más: manda la \hl{resolución de entrada}.}
\end{frame}
...
\begin{frame}{Por qué ImageNet no ayuda en un tablero sintético}
  \begin{itemize}
    ...
    \item V0 guarda esa geometría con \emph{spatial softmax}; V1 y V2 dan un descriptor
          global.
  \end{itemize}
...
\begin{frame}{Dónde falla este estudio}
  \begin{itemize}
    \item Entre V0 y V1--V2 cambian \hl{cuatro factores a la vez}: inicialización,
          resolución, recorte y agregación espacial.
    \item Una sola semilla: los intervalos cubren episodios, no réplicas.
    \item Sin V3 ni V4, convolucionales frente a ViT sigue abierto.
  \end{itemize}
  \takeaway{Se compara el bloque codificador\,+\,preprocesado, no la inicialización
    aislada.}
\end{frame}
...
\begin{frame}{Para llevarse a casa}
  \begin{enumerate}
    \item Con \num{206} demostraciones de Push-T, la ResNet-18 desde cero ganó por
          \num{0.197} y \num{0.217} puntos.
    \item Ajustar los pesos preentrenados no aportó ventaja sobre congelarlos
          ($p=\num{0.82}$).
    \item Siguiente paso: cerrar V3 y V4; entrenar desde cero a \num{224} px.
  \end{enumerate}
  \vfill
  {\color{tsMute}\small Bifurcación propia de \texttt{diffusion\_policy} ·
    \num{1266} épocas ejecutadas}
\end{frame}
...
\begin{frame}[noframenumbering]{Media de las tres últimas evaluaciones}
  ...
  \begin{tabular}{l S[table-format=1.3] S[table-format=1.3] S[table-format=3.2]}
    ...
    \bottomrule
  \end{tabular}
  \src{V2 logra el menor error sobre acciones del conjunto de datos y aun así no gana la
    tarea: precisión en cadena abierta y control en bucle cerrado no miden lo mismo.}
\end{frame}
