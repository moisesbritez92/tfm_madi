...
El argumento que acota un sesgo de selecci\'on es el procedimiento, no una prueba
estad\'istica: el reparto responde a un sorteo aleatorio uniforme sobre los \'indices de
episodio, sin ning\'un criterio de calidad, longitud ni dificultad. El procedimiento no
introduce, por tanto, un criterio sistem\'atico de selecci\'on. Eso vale en expectativa y no
por realizaci\'on: una extracci\'on concreta s\'i puede producir un desequilibrio muestral
por azar, y de ah\'i que las comparaciones descriptivas que siguen conserven su utilidad.
...
Durante el entrenamiento se conservan los tres puntos de control de mayor puntuaci\'on de
evaluaci\'on. El modelo evaluado en el \cref{cap:resultados} es el de mayor puntuaci\'on,
criterio coherente con el empleado en la literatura de la tarea
\cite{mandlekar2021robomimic}.

Conviene separar dos frecuencias que la \cref{tab:config-efectiva} recoge por separado y que
no significan lo mismo. La fila de guardado indica cada cu\'antas \'epocas se reescribe el
estado t\'ecnico que permite reanudar la ejecuci\'on, \num{50} \'epocas en V0, V1 y V2 y
\num{10} en V3 y V4. La selecci\'on, en cambio, se decide sobre la puntuaci\'on de
evaluaci\'on, que el simulador solo produce cada \num{50} \'epocas en las cinco variantes. El
guardado de los tres mejores est\'a condicionado a que esa puntuaci\'on exista en la \'epoca
en curso, de modo que \textbf{solo las \'epocas m\'ultiplo de \num{50} generan candidatos a
la selecci\'on}, tambi\'en en V3 y V4. Los puntos de control conservados de esas dos
variantes lo confirman: corresponden a las \'epocas \num{50}, \num{100} y \num{150}.
...
sobre el generador de condiciones iniciales de Push-T \textbf{y sobre la realizaci\'on del
ruido de difusi\'on}. Se estima con las \num{200} condiciones del bloque de prueba,
muestreadas de ese generador mediante semillas consecutivas, cada una resuelta con dos
realizaciones del ruido. La variable de resultado secundaria es la probabilidad de \'exito,
estimada como la fracci\'on media de realizaciones cuya puntuaci\'on alcanza el umbral.
...
que el codificador no consume n\'umeros aleatorios.

Cada condici\'on se resuelve con \textbf{dos} realizaciones del ruido de difusi\'on,
obtenidas a partir de dos semillas base. Los n\'umeros aleatorios comunes se mantienen
dentro de cada realizaci\'on, de manera que las cinco variantes siguen siendo comparables
par a par. Las dos realizaciones se promedian dentro de cada condici\'on antes de agregar y
de contrastar, de modo que la unidad de observaci\'on sigue siendo la condici\'on inicial y
el an\'alisis opera sobre \num{200} valores independientes, no sobre \num{400}
observaciones anidadas.

Esta duplicaci\'on constituye una \textbf{modificaci\'on posterior al preregistro} de la
prueba final, que hab\'ia fijado una sola trayectoria por condici\'on, y como tal se
declara. Tres razones la sostienen. En primer lugar, el estimando que integra condici\'on y
ruido no se formula aqu\'i por primera vez: qued\'o registrado en el protocolo del
\cref{sec:comparacion-paper}, cerrado antes de observar ning\'un dato de ese contraste. En
segundo lugar, la ampliaci\'on se aplica de forma uniforme a las cinco variantes y a las
diez comparaciones, con la misma semilla base adicional, sin seleccionar ninguna en
funci\'on de su resultado. En tercer lugar, la motiva una medida y no una preferencia: la
puntuaci\'on media de V0 se desplaz\'o \num{0.030} puntos al cambiar \'unicamente esa
semilla, magnitud comparable a las diferencias que separan a las variantes preentrenadas
entre s\'i. Las cifras correspondientes a una sola realizaci\'on permanecen accesibles en
los datos publicados del trabajo, y el \cref{sec:limitaciones} recoge el alcance de este
cambio.

Dos realizaciones acotan esa fuente de variaci\'on, pero no la eliminan. La media de dos
valores conserva la mitad de la varianza del ruido, de modo que el procedimiento reduce el
problema sin resolverlo por completo.
...
resultado principal, y cada contraste se corrige sobre su propia familia de diez. Sus
valores $p$ se ajustan mediante el procedimiento secuencial de Holm, que controla el error
familiar sin suponer independencia entre contrastes, y la significaci\'on se determina sobre
los valores ajustados con un umbral de \num{0.05}. Los intervalos de confianza no incorporan
ese ajuste y se presentan como descriptivos.

La probabilidad de \'exito es una variable secundaria y descriptiva: no entra en ninguna
familia corregida ni sostiene conclusi\'on alguna por s\'i sola. Su intervalo procede del
mismo \textit{bootstrap} BCa sobre las \num{200} condiciones, y no del intervalo de Wilson
para una proporci\'on binomial, puesto que las \num{400} observaciones que la componen no
son independientes: las dos de una misma condici\'on comparten la pose inicial. Tampoco se
umbraliza la puntuaci\'on ya promediada, porque el promedio de dos indicadores binarios no
constituye un indicador.

Estas medidas caracterizan la variabilidad debida a las condiciones iniciales y a la
realizaci\'on del ruido de difusi\'on, las dos fuentes que el estimando integra. No
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{30mm} p{40mm} p{40mm}}
    ...
    \bottomrule
  \end{tabular}
  \notatabla{Los factores de la mitad inferior var\'ian de forma conjunta, de modo que el
    contraste no aisla ninguno de ellos por separado.}
  \fuente{elaboraci\'on propia}
\end{table}

Esos factores var\'ian a la vez. El alcance del contraste es, en consecuencia, el de una
comparaci\'on entre dos artefactos completos: determina si el modelo obtenido en este
trabajo rinde o no por debajo del publicado, no a qu\'e componente se debe la diferencia. La
unidad sobre la que se infiere sigue siendo el artefacto entrenado.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{32mm} p{40mm} p{40mm}}
    ...
    IC \SI{90}{\percent} fuera de $\pm\delta$ & Diferencia detectable, relevancia
      pr\'actica no establecida & Indeterminado, por potencia insuficiente \\
    \bottomrule
  \end{tabular}
  \notatabla{El valor $p$ procede de la prueba de permutaci\'on y el intervalo del
    \textit{bootstrap} BCa de \num{10000} remuestreos. Las dos casillas de la fila inferior
    est\'an acotadas en sentidos opuestos y ninguna de las dos debe leerse de m\'as. La
    derecha no autoriza a concluir equivalencia, puesto que la ausencia de significaci\'on y
    la imprecisi\'on del intervalo son compatibles con diferencias mayores que el margen. La
    izquierda tampoco autoriza a concluir que la diferencia supere el margen: no haber
    demostrado equivalencia no demuestra relevancia pr\'actica, que exigir\'ia un l\'imite
    inferior del intervalo situado por encima de $+\delta$.}
  \fuente{elaboraci\'on propia}
\end{table}
...
\label{sec:transferencia-godot}

El \cref{sec:comparacion-paper} contrasta dos artefactos sobre una distribuci\'on de
im\'agenes fija, la que produce el mismo renderizador que gener\'o las demostraciones.
Ninguna de las medidas anteriores indica, en cambio, qu\'e ocurre cuando esa distribuci\'on
cambia. El presente apartado fija el protocolo de un contraste que la modifica, registrado
en el repositorio antes de ejecutar la medida.

La pregunta admite una hip\'otesis concreta, y no se plantea como exploraci\'on abierta. El
codificador de V\textsubscript{pub} localiza posiciones mediante un \textit{spatial softmax},
mientras que el de V0 promedia la respuesta convolucional sobre toda la imagen. Dos
mecanismos tan distintos no tienen por qu\'e degradarse igual cuando cambia el renderizador,
de modo que el contraste puede separar artefactos que la evaluaci\'on original no separa.

\divisionmenor{El simulador alternativo y sus dos condiciones}

La tarea se reimplement\'o sobre el motor Godot 4.7.2 \cite{linietsky2014godot}, con f\'isica propia del motor y
geometr\'ia portada desde el entorno original. La simulaci\'on se resuelve en dos
dimensiones, pese a que la escena se representa en tres: con gravedad y rozamiento de mesa
la pieza volcar\'ia y la tarea dejar\'ia de ser la que los artefactos aprendieron. El paso de
control conserva los diez subpasos de integraci\'on del entorno original, as\'i como los
horizontes de observaci\'on y ejecuci\'on y el l\'imite de \num{300} pasos.

Sobre ese port se definen las dos condiciones de la \cref{tab:godot-condiciones}, que
difieren \'unicamente en el origen de la imagen que recibe la pol\'itica.

\begin{table}[htbp]
  \centering
  \caption{Condiciones de observaci\'on del simulador alternativo.}
  \label{tab:godot-condiciones}
  \small
  \scriptsize
  \begin{tabular}{p{16mm} p{44mm} p{50mm}}
    \toprule
    Condici\'on & Origen de la imagen & Qu\'e cambia respecto del entrenamiento \\
    \midrule
    A & Rasterizador del entrenamiento, en Python, alimentado con el estado que
      env\'ia Godot & Solo el motor de f\'isica \\
    B & Vista ortogonal cenital del propio Godot & El motor de f\'isica y los p\'ixeles \\
    \bottomrule
  \end{tabular}
  \notatabla{La condici\'on B es la primaria. La A act\'ua como control: sin ella, una
    degradaci\'on observada en B no podr\'ia repartirse entre el cambio de din\'amica y el
    cambio de apariencia.}
  \fuente{elaboraci\'on propia}
\end{table}

Los puntos de control son los mismos dos del \cref{sec:comparacion-paper}, verificados por
resumen criptogr\'afico antes de cada corrida. No se ajusta ni se selecciona nada.

\divisionmenor{Dise\~no y bloque de evaluaci\'on}

Los dos artefactos se eval\'uan sobre las \num{50} condiciones iniciales
\numrange{200000}{200049}, prefijo del bloque final, de modo que cada condici\'on dispone de
una cifra de referencia ya medida en el simulador original. Al cruzar dos artefactos, dos
condiciones de observaci\'on y dos realizaciones del ruido de difusi\'on resultan ocho celdas
y \num{400} episodios.

Interesa subrayar una decisi\'on de muestreo. Las semillas \textbf{no se condicionan al
\'exito previo} de ning\'un artefacto. Ese filtro, habitual cuando se busca observar una
degradaci\'on, elevar\'ia las cifras absolutas y hurtar\'ia al contraste las condiciones que
ning\'un artefacto resuelve. El bloque se toma, por tanto, completo y por orden.

Las dos realizaciones del ruido emplean las semillas base ya declaradas en el
\cref{sec:comparacion-paper} y se promedian dentro de cada condici\'on antes de contrastar,
de manera que el an\'alisis opera sobre \num{50} diferencias independientes. El estimando
primario es la diferencia entre las puntuaciones medias de los dos artefactos en la
condici\'on B, integrada sobre el generador de condiciones iniciales y sobre la realizaci\'on
del ruido.

Conviene registrar aqu\'i una limitaci\'on propia de este simulador, medida con antelaci\'on.
Los episodios no se repiten de forma id\'entica entre ejecuciones: una misma condici\'on
resuelta dos veces con el mismo punto de control dio \num{0.9726} en \num{183} pasos y
\num{0.9545} en \num{242}. El ruido de difusi\'on s\'i queda sembrado, pero las convoluciones
en la unidad gr\'afica no son deterministas y una simulaci\'on con contactos amplifica
cualquier discrepancia diminuta. En consecuencia, la varianza que separa las dos
realizaciones mezcla el cambio de semilla con ese indeterminismo y no puede atribuirse a uno
de los dos. As\'i se reporta.

\divisionmenor{Contraste, margen y potencia}

El contraste primario, el margen de equivalencia y la regla de decisi\'on son los del
\cref{sec:comparacion-paper}, sin cambios: prueba de permutaci\'on por inversi\'on de signo
con \num{10000} permutaciones, intervalo BCa, margen $\delta = \num{0.05}$ y las cuatro
casillas de la \cref{tab:regla-decision}. Existe un \'unico contraste primario, de modo que
no hay familia de hip\'otesis que corregir. Los contrastes de la condici\'on A, la ca\'ida de
A a B, la deriva respecto del simulador original, la tasa de \'exito y la componente de
varianza del ruido se reportan como secundarios, sin correcci\'on y etiquetados como tales.

La potencia del dise\~no se calcul\'o antes de observar dato alguno, y su resultado obliga a
una advertencia expl\'icita. Con la desviaci\'on t\'ipica de las diferencias pareadas
estimada en el simulador original, aproximadamente \num{0.267}, un tama\~no de \num{50}
condiciones con dos realizaciones promediadas arroja un error est\'andar cercano a
\num{0.038}. El efecto m\'inimo detectable a dos colas y con potencia del \SI{80}{\percent}
asciende, por tanto, a unos \num{0.105} puntos.

De ah\'i se siguen dos consecuencias que se declaran de antemano. La primera es que el
dise\~no no est\'a dimensionado para reproducir la diferencia de \num{0.041} medida en el
simulador original, de manera que no detectarla no constituir\'ia evidencia de su ausencia.
La segunda es que el intervalo al \SI{90}{\percent} mide alrededor de \num{0.125} puntos y no
cabe dentro de $\pm\num{0.05}$: \textbf{este dise\~no no puede declarar equivalencia
pr\'actica}, cualquiera que sea el dato. La casilla correspondiente de la
\cref{tab:regla-decision} queda fuera de alcance, y no poder alcanzarla no ser\'a un
resultado. El tama\~no lo impone el coste, ya que la condici\'on B exige renderizador y no
admite paralelizaci\'on.

\divisionmenor{Comprobaciones previas y entorno}

Tres comprobaciones con criterio num\'erico fijado de antemano preceden a la medida. La
primera verifica los resumenes criptogr\'aficos de los dos puntos de control y la disyunci\'on
del bloque respecto de las demostraciones y de los dos conjuntos de selecci\'on. La segunda
acota la deriva de entorno: cada artefacto se eval\'ua sobre las ocho primeras condiciones
del bloque contra el simulador original, y se exige que la media se aleje menos de \num{0.07}
de la cifra ya registrada. La tercera comprueba que el artefacto de referencia cierra el
bucle en la condici\'on B sin que se dispare el aviso de historial de observaciones
congelado, cuyo \'unico s\'intoma ser\'ia una puntuaci\'on algo peor.

La segunda comprobaci\'on requiere una precisi\'on sobre su alcance. El servidor de
inferencia siembra el ruido indexando por condici\'on inicial, mientras que el evaluador del
\cref{sec:prueba-final} lo indexa por grupo, de modo que la corriente de ruido no coincide y
una igualdad exacta resulta imposible por construcci\'on. Lo que la comprobaci\'on acota es,
en consecuencia, el efecto conjunto de cambiar de entorno de ejecuci\'on y de corriente de
ruido, con la f\'isica fijada en el simulador original. Basta para el prop\'osito que se le
asigna, que es separar lo atribuible al simulador alternativo de todo lo dem\'as.

A diferencia del \cref{sec:comparacion-paper}, la ejecuci\'on se realiza \'integramente en
el entorno de inferencia de Windows, puesto que el motor Godot corre en esa plataforma y el
servidor de pol\'itica debe comunicarse con \'el en la misma m\'aquina. La exigencia de fondo
se respeta igualmente: \textbf{los dos artefactos comparten entorno}, de modo que el cambio
de versi\'on de las bibliotecas es com\'un a ambos y no interviene en la diferencia pareada.

\divisionmenor{Alcance}

Los cuatro factores confundidos del \cref{sec:comparacion-paper} lo siguen estando. Este
contraste no aisla ninguno: a\~nade un quinto eje, el dominio de evaluaci\'on, y lo que mide
de forma limpia es la degradaci\'on de cada artefacto respecto de s\'i mismo. La unidad sobre
la que se infiere contin\'ua siendo el artefacto entrenado.
...
permanecen sin cambios. Las modificaciones arquitect\'onicas se concentran, por tanto, en el
bloque visual y no en la infraestructura que lo rodea. Eso no convierte al codificador en el
\'unico factor que var\'ia: las desviaciones experimentales que no se pudieron igualar, esto
es, el presupuesto de \'epocas, el tama\~no f\'isico de lote, la acumulaci\'on de gradiente y
la frecuencia de guardado, se detallan en la \cref{tab:config-efectiva}. El c\'odigo
modificado, las configuraciones de las cinco variantes, los guiones de an\'alisis y el
protocolo de la
