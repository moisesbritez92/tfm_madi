...
El estudio adopta un dise\~no comparativo controlado con una \'unica variable manipulada.
Esa variable es el codificador visual de la pol\'itica, entendido como la combinaci\'on de
una arquitectura de red y una estrategia de entrenamiento de sus pesos. El resto del
sistema permanece id\'entico en todas las condiciones.
...
Para que la comparaci\'on resulte interpretable, se mantienen constantes seis factores: la
red generativa y sus hiperpar\'ametros, el conjunto de datos y su partici\'on, la semilla de
entrenamiento, el presupuesto m\'aximo de \'epocas y su criterio de parada, el protocolo de
evaluaci\'on y la dimensi\'on del vector de condicionamiento a la salida del codificador.
...
variante se entrena con una \'unica semilla, por lo que las diferencias observadas no
admiten contraste estad\'istico. Esta decisi\'on responde al coste computacional, ya que un
entrenamiento ocupa entre \num{17} y \num{96} horas de la \'unica unidad de procesamiento
...
La partici\'on de los datos reproduce la de la implementaci\'on de referencia. Un dos por
ciento de los episodios se reserva para validaci\'on y el entrenamiento se limita a
\num{90} episodios; los restantes se descartan. Mantener este criterio permite comparar los
resultados con los publicados y, adem\'as, sit\'ua el experimento en el r\'egimen de pocas
demostraciones que motiva las hip\'otesis del \cref{sec:planteamiento}.
...
Se definen cinco variantes, identificadas de V0 a V4, que difieren \'unicamente en el
codificador visual y en el tratamiento de sus pesos. La \cref{tab:variantes} resume su
configuraci\'on. Las tres primeras comparten arquitectura convolucional y aislan el efecto
de la estrategia de entrenamiento; las dos \'ultimas introducen transformadores de
visi\'on preentrenados con objetivos distintos.
...
\cite{radford2021clip}. En ambos casos los pesos permanecen congelados, ya que el ajuste
fino de un transformador no cabe en la memoria gr\'afica disponible.
...
Todas las variantes comparten los hiperpar\'ametros recogidos en la
\cref{tab:hiperparametros}, heredados de la configuraci\'on oficial del m\'etodo. El
presupuesto m\'aximo se fij\'o en \num{500} \'epocas, frente a las \num{3000} del
art\'iculo original, por limitaciones de c\'omputo. Cada entrenamiento se detiene antes de
agotar ese presupuesto cuando deja de mejorar, criterio que se precisa m\'as adelante en
este mismo apartado. En consecuencia, las variantes completan entre \num{200} y \num{500}
\'epocas.

\begin{table}[htbp]
  \centering
  \caption{Hiperpar\'ametros comunes a las cinco variantes.}
...
El tama\~no de lote de V4 se redujo a la mitad porque CLIP ViT-B/16 no cabe en los ocho
gigabytes de memoria gr\'afica con lotes de \num{64} muestras. Se trata de la \'unica
desviaci\'on respecto a la configuraci\'on com\'un y afecta al n\'umero de actualizaciones
por \'epoca, extremo que se retoma al interpretar los resultados.
...
Este criterio no precedi\'o al experimento, sino que se adopt\'o durante su ejecuci\'on, y
la memoria lo describe as\'i por exactitud. Las dos primeras variantes lanzadas, V0 y V1,
agotaron el presupuesto de \num{500} \'epocas porque entonces no exist\'ia regla alguna de
parada. La formulaci\'on anterior se fij\'o al observar sus curvas y el coste de las
\'epocas improductivas que recoge el \cref{sec:coste}, y solo se aplic\'o a V2, detenida
en la \'epoca \num{266}. La decisi\'on se tom\'o adem\'as por inspecci\'on visual, no de
forma autom\'atica.

Esa asimetr\'ia afecta a la comparabilidad del coste, no a la del rendimiento, puesto que
el mejor punto de control de cada variante procede de una regi\'on de la curva ya
explorada. El \cref{sec:coste} cuantifica el coste que habr\'ian tenido las tres
ejecuciones bajo el criterio aplicado de forma uniforme.
...
de la acci\'on predicha y la puntuaci\'on media sobre las condiciones de entrenamiento.
Las dos primeras describen el ajuste, mientras que la comparaci\'on entre las dos
puntuaciones permite detectar sobreajuste.
...
independiente. El t\'ermino \textit{prueba} se reserva en esta memoria para una medida
sobre condiciones ajenas a la selecci\'on, que este trabajo no realiza; el sesgo que se
deriva de ello se analiza en el \cref{sec:limitaciones}.

La dispersi\'on entre episodios permite acompa\~nar cada diferencia observada de un
intervalo de confianza y contrastarla mediante la prueba de los rangos con signo de
Wilcoxon, aplicada sobre los pares de episodios comunes a dos variantes. Ambas medidas
caracterizan la variabilidad debida a las condiciones iniciales del simulador. No
...
fase de calentamiento. El tercero es la latencia de inferencia para un lote de tama\~no
fijo. El cuarto es el pico de memoria gr\'afica reservada durante el entrenamiento.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    \toprule
    Variante & \'Epocas completas & Bucle por \'epoca & Bucle acumulado & Tiempo total \\
    \midrule
    V0 & \num{500} & \SI{1.5}{\minute}  & \SI{12.4}{\hour}$^{\dagger}$ & \SI{17.4}{\hour} \\
    V1 & \num{500} & \SI{5.3}{\minute}  & \SI{43.8}{\hour}             & \SI{96.5}{\hour} \\
    V2 & \num{266} & \SI{14.7}{\minute} & \SI{65.3}{\hour}$^{\dagger}$ & \SI{84.9}{\hour}$^{\dagger}$ \\
    V3 & \multicolumn{4}{c}{en ejecuci\'on} \\
    V4 & \multicolumn{4}{c}{en ejecuci\'on} \\
    \midrule
    Total (V0--V2) & \num{1266} & & \SI{121.6}{\hour} & \SI{198.8}{\hour} \\
    \bottomrule
  \end{tabular}
...
Las tres variantes completadas suman casi \num{200} horas de c\'omputo, muy por encima de
las \num{75} horas que cabr\'ia proyectar a partir de la primera ejecuci\'on. La
discrepancia procede de dos factores. En primer lugar, el coste por \'epoca crece un orden
de magnitud entre V0 y V2, ya que las variantes preentrenadas operan sobre im\'agenes de
\num{224} p\'ixeles en lugar de \num{96}. En segundo lugar, la validaci\'on y las
evaluaciones en el simulador llegan a duplicar el tiempo del bucle de optimizaci\'on,
puesto que la simulaci\'on de los episodios se ejecuta en el procesador y no en la GPU.
...
Tres aclaraciones acotan la lectura de la tabla. En primer lugar, V0 y V2 conservan el
cron\'ometro de una parte de sus \'epocas, no de todas, de modo que sus tiempos acumulados
se extrapolan a partir de la media por \'epoca; el s\'imbolo $\dagger$ marca esos valores.
En segundo lugar, V2 se ejecut\'o en dos sesiones separadas por una interrupci\'on de dos
d\'ias, cuya duraci\'on se descuenta del tiempo total por no corresponder a c\'omputo
efectivo. En tercer lugar, la columna de \'epocas recoge las completadas: V2 registra una
\'epoca adicional que qued\'o interrumpida y que no se contabiliza aqu\'i ni en el
\cref{cap:resultados}.
...
del orden de \num{1000} horas de la misma GPU. La segunda es la adopci\'on del criterio de
...
Puesto que ese criterio solo lleg\'o a aplicarse a V2, la tabla mide el experimento tal
como se ejecut\'o y no el protocolo declarado. La diferencia se reconstruye a partir de los
registros de evaluaci\'on. Bajo el criterio, V0 se habr\'ia detenido en la \'epoca
\num{300} y V1 en la \num{250}, de modo que el bucle de optimizaci\'on habr\'ia sumado unas
\SI{91}{\hour} en lugar de \SI{121.6}{\hour} y el tiempo total unas \SI{139}{\hour} en
lugar de \SI{198.8}{\hour}, esto es, un ahorro pr\'oximo al \SI{30}{\percent}. El punto de
